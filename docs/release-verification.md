...
```powershell
cosign verify-blob `
  --bundle .\Tkya-v1.0.3-android-arm64-v8a.apk.sigstore.json `
  --certificate-identity "https://github.com/andyhz0823/kuaifei/.github/workflows/release.yml@refs/tags/v1.0.3" `
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" `
  .\Tkya-v1.0.3-android-arm64-v8a.apk
```

For another normal tag-triggered release, replace `v1.0.3` in both the filename and the certificate identity. Verify the corresponding `.exe`, `.zip`, `SHA256SUMS.txt`, and `SIGSTORE-VERIFICATION.txt` in the same way.
