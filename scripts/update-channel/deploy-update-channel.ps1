...
.PARAMETER Tag
  Release 标签，例如 v1.0.2。

.EXAMPLE
  # 推荐：私钥路径走环境变量，避免把本机路径写进脚本
  $env:TKYA_DEPLOY_SSH_KEY = 'D:\path\to\deploy.pem'
  .\deploy-update-channel.ps1 -Tag v1.0.2

.EXAMPLE
  .\deploy-update-channel.ps1 -Tag v1.0.2 -SshKeyPath 'C:\keys\deploy.pem' -SkipVerify
#>
[CmdletBinding()]
param(
